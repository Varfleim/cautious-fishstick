...
{
    public class SHexasphere : IEcsRunSystem
    {
        //Миры
            ...
            ref RMapGeneration mapGenerationRequest)
        {
            ...
            mapGenerationData.Value.regionHighlightMaterial.shaderKeywords = new string[] { ShaderParameters.SKW_HIGHLIGHT_TINT_BACKGROUND };
            mapGenerationData.Value.regionHighlightMaterial.SetFloat(ShaderParameters.ColorShift, 1f);
            ...
            int regionIndex)
        {
            ...
            GameObject gO = MapCreateGOAndParent(parentGO, "Region");
            MeshFilter mf = gO.AddComponent<MeshFilter>();
            Mesh mesh = new();
            mesh.hideFlags = HideFlags.DontSave;

            //Определяем высоту рендерера
            float extrusionAmount = currentRegion.ExtrudeAmount * MapGenerationData.extrudeMultiplier;

            //Рассчитываем выдавленные вершины региона
            Vector3[] extrudedVertices = new Vector3[currentRegion.vertexPoints.Length];
            //Для каждой вершины
            for (int a = 0; a < currentRegion.vertices.Length; a++)
            {
                //Рассчитываем положение выдавленной вершины
                extrudedVertices[a] = currentRegion.vertices[a] * (1f + extrusionAmount);
            }
            //Назначаем вершины мешу
            mesh.vertices = extrudedVertices;

            //Если у региона шесть вершин
            if (currentRegion.vertices.Length == 6)
            {
                mesh.SetIndices(
            ...
            mesh.normals = currentRegion.vertices;
            ...
            currentRegion.renderer = mr;
            currentRegion.renderer.enabled = false;
