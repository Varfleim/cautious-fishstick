...
namespace SandOcean.Warfare.Fleet
{
    public class SFleetControl : IEcsRunSystem
    {
            ...
            ref RFleetCreating requestComp)
        {
            ...
            requestComp.ownerOrganizationPE.Unpack(world.Value, out int organizationEntity);
            ...
            ref RTaskForceCreating requestComp)
        {
            ...
            UnityEngine.Debug.LogWarning("Создана оперативная группа!");
            ...
            ref RTFTemplateAction requestComp)
        {
            ...
            foreach (int templateEntity in tFTemplateDisplayedSummaryPanelFilter.Value)
            {
                ...
                if (templateDisplayedSummaryPanel.CheckTemplate(template))
                {
                    //Запрашиваем удаление интерфейса
                    TFTemplateRefreshUISelfRequest(
                        templateEntity,
        ...
        void TFTemplateRefreshUISelfRequest(
            int templateEntity,
            UI.RefresUIType requestType)
        {
            //Назначаем сущности компонент события обновления интерфейса
            ref SRTFTemplateRefreshUI selfRequestComp = ref tFTemplateRefreshUISelfRequestPool.Value.Add(templateEntity);
