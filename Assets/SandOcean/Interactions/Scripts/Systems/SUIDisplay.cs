...
namespace SandOcean.UI
{
    public class SUIDisplay : IEcsInitSystem, IEcsRunSystem
    {
        ...
        public void Init(IEcsSystems systems)
        {
            ...
            eUI.Value.designerWindow.shipDesigner.availableComponentTypeDropdown.ClearOptions();
            //Заполняем список типов компонентов в дизайнере кораблей
            eUI.Value.designerWindow.shipDesigner.availableComponentTypeDropdown.AddOptions(new List<string>()
            {
            ...
            eUI.Value.designerWindow.shipDesigner.currentAvailableComponentsType = 0;
            //ТЕСТ
        }
        ...
        public void Run(IEcsSystems systems)
        {
                ...
                == MainWindowType.Game)
            {
                ...
                foreach (int selfRequestEntity in refreshRAEOObjectPanelSelfRequestFilter.Value)
                {
                    ...
                    if (gameWindow.objectPanel.activeObjectSubpanelType == ObjectSubpanelType.Region)
                    {
                        ...
                        {
                            //Берём панель RAEO
                            UIRegionObjectSubpanel RAEOSubpanel = gameWindow.objectPanel.regionObjectSubpanel;
                            ...
                            else if (RAEOSubpanel.tabGroup.selectedTab == RAEOSubpanel.organizationsTab.selfTabButton)
                            {
                ...
                in regionFilter.Value)
            {
                ...
                //Если в регионе есть группы кораблей
                if (region.shipGroups.Count > 0)
                {
                    ...
                    {
                        //Берём компонент группы кораблей и её MO
                        currentShipGroupNode.Value.Unpack(world.Value, out int shipGroupEntity);
                        ...
                        //Переписать то, как GO корабля назначается орбитальному объекта

                        //Если SO корабля не является дочерним SO родительского ОО
                        if (shipSO.Transform.parent != regionSO.Transform)
                        {
                            Debug.LogWarning("!");

                            //Назначаем его SO родительского ОО
                            shipSO.Transform.SetParent(regionSO.Transform);
                        }

                        //Перемещаем GO группы кораблей
                        shipGroupMO.LocalPosition = shipGroup.position;

                        //Берём следующий корабль в списке
                        currentShipGroupNode = currentShipGroupNode.Next;
                    }
                }
            }*/

            foreach (int shipGroupEntity in shipGroupFilter.Value)
            {
                //Берём компонент группы кораблей и её MO
                ref CShipGroup shipGroup = ref shipGroupPool.Value.Get(shipGroupEntity);
                ref CMapObject shipGroupMO = ref mapObjectPool.Value.Get(shipGroupEntity);
            ...
            EcsPackedEntity factionPE = new EcsPackedEntity())
        {
                ...
                == DesignerType.ShipClass)
            {
                ...
                designerWindow.shipDesigner.gameObject.SetActive(
                    true);
                //Указываем его как активный дизайнер в окне дизайнера
                designerWindow.activeDesigner 
                    = designerWindow.shipDesigner.gameObject;
            ...
            ref RGameDisplayObjectPanel gameDisplayObjectPanelRequest)
        {
            ...
            //Если какая-либо подпанель активна
            ...
            objectPanel.regionObjectSubpanel.gameObject.SetActive(true);
        ...
        void GameDisplayRegionOSbPOverviewTab(
            ref CHexRegion region,
            ref CRegionAEO rAEO,
            bool isRefresh)
        {
            //Берём подпанель региона
            UIRegionObjectSubpanel regionSubpanel = eUI.Value.gameWindow.objectPanel.regionObjectSubpanel;

            //Берём обзорную вкладку
            UIRegionOSbPOverviewTab overviewTab = regionSubpanel.overviewTab;
        ...
        void GameDisplayRegionOSbPOrganizationsTab(
            ...
            bool isRefresh)
        {
            ...
            UIRegionOSbPOrganizationsTab organizationsTab = regionSubpanel.organizationsTab;
            ...
            for (int a = 0; a < regionRAEO.organizationRAEOs.Length; a++)
            {
                ...
                {
                    //Берём панель ORAEO 
                    UIORAEOBriefInfoPanel briefInfoPanel = organizationsTab.panelsList[a];
                    ...
                    if (regionRAEO.organizationRAEOs[a].organizationRAEOType == ORAEOType.Economic)
                    {
                        ...
                        //Указываем PE ORAEO
                        briefInfoPanel.organizationRAEOPE = exORAEO.selfPE;
                    }
                    //Иначе
                    else
                    {
            ...
            ref RGameDisplayObjectPanel gameDisplayObjectPanelRequest)
        {
            ...
            objectPanel.oRAEOObjectSubpanel.gameObject.SetActive(true);
        ...
        void GameDisplayORAEOOSbPOverviewTab(
            ref CExplorationORAEO exORAEO,
            ref CEconomicORAEO ecORAEO,
            bool isRefresh)
        {
            //Берём подпанель ORAEO
            UIORAEOObjectSubpanel oRAEOSubpanel = eUI.Value.gameWindow.objectPanel.oRAEOObjectSubpanel;

            //Берём обзорную вкладку
            UIORAEOOSbPOverviewTab overviewTab = oRAEOSubpanel.overviewTab;
